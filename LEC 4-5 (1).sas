...
data pulse;
input  id pulse1 pulse2 run smokes sex height weight activity;
datalines;
...
run;
PROC PRINT data = pulse_new;
RUN;
PROC MEANS  data=pulse min max mean std;
VAR pulse1 pulse2;
RUN;

PROC MEANS data=pulse lclm uclm alpha=0.10; /* 90% Confidence Interval*/
VAR pulse1;
RUN;

PROC MEANS data=pulse lclm uclm alpha=0.05; /* 95% Confidence Interval*/
VAR pulse2 height;
RUN;



PROC TTEST data=pulse H0=70 SIDES=2 plots=none; /* Two tailed test */
VAR pulse1;
Run;

PROC TTEST data=pulse H0=70 SIDES=U plots=none;  /* Right (Upper) tailed test */
VAR pulse1;
Run;
PROC TTEST data=pulse H0=70 SIDES=L plots=none; /* Left (Lower)  tailed test */
VAR pulse1;
Run;
